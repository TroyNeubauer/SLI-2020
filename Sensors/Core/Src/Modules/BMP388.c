...
static dumb_bool bmp388Check(void);
...
{
  uint8_t u8Ret;
  uint8_t command = RegAddr | 0b10000000;
  HAL_GPIO_WritePin(ALT_CS_GPIO_Port, ALT_CS_Pin, GPIO_PIN_RESET);
  HAL_Delay(1);
  HAL_SPI_Transmit(&hspi1, &command, 1, HAL_MAX_DELAY);
  //HAL_SPI_Receive(&hspi1, &u8Ret, 1, HAL_MAX_DELAY); //dummy byte
  HAL_SPI_Receive(&hspi1, &u8Ret, 1, HAL_MAX_DELAY);
  HAL_Delay(1);
