...
{
	status = ICM_20948_set_sample_mode(&_device, (ICM_20948_InternalSensorID_bm) sensor_id_bm,
...
ICM_20948_Status_e ICM_20948::clearInterrupts(void)
{
	...
	status = ICM_20948_execute_r(&_device, AGB0_REG_INT_STATUS_1, (uint8_t*) &int_stat_1,
	        sizeof(ICM_20948_INT_STATUS_1_t));
	if (status != ICM_20948_Stat_Ok)
	{
		return status;
	}
...
ICM_20948_Status_e ICM_20948::i2cMasterConfigureSlave(uint8_t slave, uint8_t addr, uint8_t reg, uint8_t len, bool Rw,
        bool enable, bool data_only, bool grp, bool swap)
{
	status = ICM_20948_i2c_master_configure_slave(&_device, slave, addr, reg, len, Rw, enable, data_only, grp, swap);
	return status;
}

ICM_20948_Status_e ICM_20948::i2cMasterSLV4Transaction(uint8_t addr, uint8_t reg, uint8_t *data, uint8_t len, bool Rw,
        bool send_reg_addr)
{
...
ICM_20948_Status_e ICM_20948_SPI::begin(SPI_HandleTypeDef *spi, GPIO_TypeDef *bank, uint16_t csPin)
{
	// Associate
	m_SPI = spi;
	m_Bank = bank;
	...
	// Set pins to default positions
	HAL_GPIO_WritePin(m_Bank, m_CSPin, GPIO_PIN_SET);
...
ICM_20948_Status_e ICM_20948_write_SPI(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
	...
	SPI_HandleTypeDef* spi = ((ICM_20948_SPI*) user)->m_SPI; // Cast user field to ICM_20948_SPI type and extract the SPI interface pointer
	GPIO_TypeDef* bank = ((ICM_20948_SPI*) user)->m_Bank;
	uint16_t cs = ((ICM_20948_SPI*) user)->m_CSPin;
	if (spi == NULL)
	{
		return ICM_20948_Stat_ParamErr;
	}
...
*/
	...
	HAL_GPIO_WritePin(bank, cs, GPIO_PIN_RESET);

	HAL_SPI_Transmit(spi, &reg, sizeof(reg), HAL_MAX_DELAY);

	HAL_SPI_Transmit(spi, data, len, GPIO_PIN_RESET);

	// delayMicroseconds(5);
	//digitalWrite(cs, HIGH);

	HAL_GPIO_WritePin(bank, cs, GPIO_PIN_SET);
...
*/
	...
	HAL_SPI_Receive(spi, buff, len, HAL_MAX_DELAY);
